...
exports.creatCity = async function (req, res, next) {
    ...
    let city = new City(_.pick(req.body, ["name", "population", "area", "touristplaces"]));
    city = await City.findOne({ name: req.body.name});
    if (city) return res.status(400).send("this city is already in database");
    city = await city.save();
    res.send(_.pick(req.body, ["name", "population", "area", "touristplaces"]));
...
exports.deletCity=async function(req,res,next){
